...
class DbmigrateController extends BaseController
{
    ...
    public function index()
    {
        ...
        $migrate->ignore(['migrations']);
        $migrate->convert('db_clouddigiturk');

        if ($migrate->write()) {
            Notification::success('Veritabanı başarılı bir şekilde aktarıldı');
        }
        else {
            Notification::danger('Veritabanı başarılı aktarılamadı. Lütfen daha sonra tekrar deneyiniz!');
        }
...
" . self::$down . "
    ...
    public function write()
    {
        ...
        return file_put_contents("../app/database/migrations/{$filename}", $schema);
    }
